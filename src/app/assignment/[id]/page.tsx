...
export default function AssignmentPage() {
        ...
        async function fetchAssignmentAndValidate() {
            ...
            try {
                ...
                if (!subSnap.empty) {
                    ...
                    // If Revise, we let them edit, but maybe pre-fill data? 
                    // For MVP simplicity in Revise mode: we just show the banner and let them start clean or continue.
                    // Ideal: Load previous state.
                }
    ...
    // If status is Completed, show the Grade Report
    if (existingSubmission?.status === 'Completed' || existingSubmission?.status === 'Pending') {
        return (
        ...
        <>
            ...
            >
                <div className={`transition-opacity duration-500 ${phase === 'SYNTHESIS' ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
                    ...
                    <TextAnnotator onAnnotate={handleAnnotate} focusedAnnotationId={focusedAnnotationId} userRole={reciprocalRole}>
                        {assignmentContent}

...
import { Loader2, Languages, Volume2, VolumeX } from "lucide-react";
...
export default function AssignmentPage() {
    ...
    // specific effect for handling translation
    useEffect(() => {
        if (isSpanish && !spanishContent && currentAssignment) {
            const fetchTranslation = async () => {
                ...
                try {
                    const res = await fetch('/api/translate', {
                        ...
                        body: JSON.stringify({ text: currentAssignment.content })
                    });
                    const data = await res.json();
                    if (data.translatedText) {
                        setSpanishContent(data.translatedText);
                    }
                } catch (error) {
                    console.error("Translation failed", error);
                    alert("Could not translate content.");
                    setIsSpanish(false);
                } finally {
    ...
    }, [isSpanish, spanishContent, currentAssignment]);
    ...
    const assignmentContent = useMemo(() => {
        ...
        const contentToRender = (isSpanish && spanishContent) ? spanishContent : currentAssignment.content;
    ...
    }, [currentAssignment, isSpanish, spanishContent]); // Added dependencies
    ...
    return (
        <>
            ...
            <DualCanvas
                sidebar={<ThinkingSidebar isSpanish={isSpanish} kwlData={kwlData} annotations={annotations} onAnnotationClick={handleAnnotationClick} />}
            ...
            >
                <div className={`transition-opacity duration-500 ${phase === 'SYNTHESIS' ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
                    ...
                    {phase === 'READING' && (
                        <div className="flex justify-end gap-2 mb-4 sticky top-0 z-40 bg-[#FDFBF7]/80 backdrop-blur-sm p-2 rounded-xl">
                            <button
                                ...
                                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all text-xs font-bold ${isSpanish ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-white text-stone-600 border-stone-200 hover:bg-stone-50'} ${translating ? 'opacity-50 cursor-wait' : ''}`}
                            >
                                <Languages size={14} />
                                {translating ? 'Translating...' : (isSpanish ? 'Español' : 'English')}
                            </button>
                    ...
                    <TextAnnotator onAnnotate={handleAnnotate} focusedAnnotationId={focusedAnnotationId}>
                        {assignmentContent}

...
import { Send, X, Copy, Plus } from 'lucide-react';
...
export default function TeacherDashboard() {
    ...
    const handleCreateClass = async (e: React.FormEvent) => {
        ...
        if (!user) {
            console.error("No user found during class creation");
        ...
        try {
            console.log("Attempting to add doc to 'classes' collection...");
    ...
    const handleSeedTestSubmission = async () => {
        if (!user || assignments.length === 0) return;
        try {
            // Need to ensure user profile exists in 'users' collection too, which it should if logged in
            const assignment = assignments[0];
            const submissionData = {
                assignmentId: assignment.id,
                studentId: user.uid,
                classId: 'test-class',
                airlockData: { primingWords: ['test'], knowledge: 'Test K', curiosity: 'Test C' },
                annotations: [{ id: '1', text: 'Test Text', note: 'Test Note', tag: 'Self-Connection' as any }], // Cast for simplicity
                synthesis: { gist: 'Test Gist', reflection3: ['A', 'B', 'C'], reflection2: ['D', 'E'], reflection1: 'Reflect', learned: 'Learned' },
                submittedAt: serverTimestamp(),
                status: 'Completed'
            };
            const docRef = await addDoc(collection(db, 'submissions'), submissionData);
            console.log("Submission seeded:", docRef.id);
            router.push(`/dashboard/review/${docRef.id}`);
        } catch (e) {
            console.error(e);
            alert("Error seeding submission");
        }
    };
                        ...
                        {isCreatingClass && (
                            <form onSubmit={handleCreateClass} className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-100 animate-in fade-in slide-in-from-top-2">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    >
                                        {isSubmittingClass ? 'Creating...' : 'Create'}
                                    </button>
                                    <button
                                        type="button"
                        ...
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className="px-3 py-1.5 bg-stone-900 text-white rounded-lg hover:bg-stone-800 transition flex items-center gap-2 text-sm shadow-sm"
                                >
                                    <Plus size={16} />
                                ...
                                <button onClick={handleSeedTestSubmission} className="text-xs text-stone-400 hover:text-stone-900 border border-stone-200 px-2 rounded">
                                    Test Sub
                                </button>
                            </div>
                        </div>
                                    ...
                                    <div key={assignment.id} className="p-4 border rounded-lg hover:shadow-md transition bg-white group relative">
                                        ...
                                        <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    router.push(`/dashboard/edit-assignment/${assignment.id}`);
                                                }}
                                                className="bg-white border border-stone-200 text-stone-500 text-xs px-3 py-1.5 rounded-full hover:text-stone-900 hover:border-stone-400 font-bold"
                                            >
                                                Edit
                                            </button>
                                            <button
                                                ...
                                                className="bg-stone-900 text-white text-xs px-3 py-1.5 rounded-full flex items-center gap-1 hover:bg-stone-700 font-bold"
                                            >
                                                Assign <Send size={12} />
                                            </button>
                                        </div>
